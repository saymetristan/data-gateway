...
5. Cálculo de relevancia por hit con cobertura ponderada por campo y poda según la policy activa.

`results[].score` está acotado a `0..1` y permite ordenar o descartar hits dentro de una respuesta. No es el `confidence` del envelope: el primero describe un record; el segundo describe la calidad del conjunto devuelto.
